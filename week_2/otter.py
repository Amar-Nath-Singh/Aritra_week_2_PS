...
class Otter(Node):
    ...
    def __init__(self,
        ...
        self.get_logger().info("Otter INIT")
        # Constants
        ...
        self.nu = self.nu                              # velocity, defined by vehicle class
        self.u_actual = self.u_actual                  # actual inputs, defined by vehicle class
        self.sampleTime = 0.05

        self.state_pub = self.create_publisher(Odometry, 'otter/odom', 10)
        self.path_pub = self.create_publisher(Path, 'otter/path', 10)
        self.action_sub = self.create_subscription(Float32MultiArray, 'otter/commands', self.action_callback, 10)
        self.loop = self.create_timer(timer_period_sec=0.1, callback = self.step)

        self.path_msg = Path()
        self.path_msg.header.frame_id = 'map'

        self.u_control = np.array([0, 0], float) # propeller revolution command
    ...
    def ode(self, action):
        u_control = np.array(action)
        self.nu, self.u_actual  = self.dynamics(self.eta,self.nu,self.u_actual,u_control,self.sampleTime)
        self.eta = attitudeEuler(self.eta,self.nu,self.sampleTime)
        return np.hstack([self.nu, self.eta])
    
    def action_callback(self, msg: Float32MultiArray):
        self.u_control = np.array(msg.data.tolist())

        self.get_logger().info(f'Prop RPM 1 :{self.u_control[1]}')
        self.get_logger().info(f'Prop RPM 2 :{self.u_control[1]}')

    def step(self):
        self.nu, self.u_actual  = self.dynamics(self.eta,self.nu,self.u_actual,self.u_control,self.sampleTime)
        self.eta = attitudeEuler(self.eta,self.nu,self.sampleTime)

        x,y,z,r,p,yaw = self.eta
        ...
        pose_msg.orientation.z) = quaternion_from_euler(r,p,yaw)

        odom = Odometry()
        odom.header.frame_id = 'map'
        odom.header.stamp = stamp
        odom.child_frame_id = 'otter'
        odom.pose.pose = pose_msg
        odom.twist.twist.linear.x,odom.twist.twist.linear.y,odom.twist.twist.linear.z = self.nu[:3]
        odom.twist.twist.angular.x,odom.twist.twist.angular.y,odom.twist.twist.angular.z = self.nu[3:]
